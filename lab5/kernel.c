...
x86_pagetable *allocate_page(int8_t owner) {
    ...
    else if (physical_page_alloc(free_page, owner) == 0) {
        return (x86_pagetable *)free_page;
    }    
...
x86_pagetable *copy_pagetable(x86_pagetable* pagetable, int8_t owner) {
	x86_pagetable *l1 = allocate_page(owner);
 	x86_pagetable *l2 = allocate_page(owner);
	if (l1 == NULL || l2 == NULL) {
        	panic("No free page table or fail to allocate");
   	 }
	memset(l1, 0, PAGESIZE);
    	l1->entry[0] = (x86_pageentry_t)l2 | PTE_P | PTE_W | PTE_U;
	memset(l2, 0, PAGESIZE);
        memcpy(l2, (x86_pagetable *)PTE_ADDR(pagetable->entry[0]), sizeof(x86_pagetable));
	return l1;
}
...
void exception(x86_registers* reg) {
	...
	if (addr < PROC_START_ADDR || addr == MEMSIZE_VIRTUAL - 1) {
            panic("Error: Invalid page address.");
